...
{
    double mmoi = 0.5 * (initial_mass - used_propellent_mass) * (0.5 * spacecraft_width) * (0.5 * spacecraft_width);
    // printf("mmoi: %lf \n", mmoi);
    return mmoi;
}
...
double cal_gimbal_angle(double torque)
{
    ...
    double _gimbal_angle = torque / thrust / moment_arm;
    ...
    double gimbal_angle = asin(_gimbal_angle);
...
{
    // moment
    double a = (1.0 / 12.0) * (initial_mass - used_propellent_mass) * (spacecraft_height * spacecraft_height);
    double b = (1.0 / 4.0) * (initial_mass - used_propellent_mass) * (spacecraft_width * spacecraft_width);
...
int main(void)
{
    ...
    while (!((propellent_mass - used_propellent_mass) < clock_period * consume_ratio))
    {
        ...
        double noise = whiteNoise(1.0);

        // double pid = pid_controller(desired_pitch, current_pitch);
        double pid = pid_controller(desired_pitch, current_pitch + noise);
